	...
	if( !isBlank( cookieVal ) && ( now <= cookieVal ) ){
		...
		if( !isBlank(weather) )
		{
			...
			$(textObj).fadeOut(200, function(){
				$(this).html("Right now, it's " + weatherJson.currently.summary + ", with a temperature of " + parseInt(weatherJson.currently.temperature) + "&deg;");
		...
		$.ajax({
			url : FORECAST_IO_URL + lat + ',' + lng + "?exclude=minutely,flags",
			dataType : 'jsonp',
			success: function (data) { 
				setStorageVal( 'local', 'weatherData', JSON.stringify(data) ); 
				setCookie( 'weatherFetchedTimestamp', null, 15 );
				$(textObj).fadeOut(200, function(){
					$(this).html( "Right now, it's " + data.currently.summary + ", with a temperature of " + parseInt(data.currently.temperature) + "&deg;" );
